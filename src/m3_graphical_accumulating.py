...
def draw_parallel_lines(n, point, length, window):
    ...
    for k in range(n):
        p1 = rg.Point(point.x, point.y + k*30) #left point
        p2 = rg.Point(point.x + length, point.y + k*30) #right point
        line = rg.Line(p1,p2) #drawing line from left to right point
        line.attach_to(window) #attach to window

    #once the for loop is done creating the lines, render the window.
    window.render()
...
def draw_lines(n, point, window):
    ...
    for k in range(n):
        if n >= 2:
            ...
            p2 = rg.Point(point.x + 100, point.y + k * change)
            line = rg.Line(p1, p2)
